    ...
    while attempt < retries do
        ...
        downloadUrlToFile(url, path, function(id, status)
            if status == dlstatus.STATUS_ENDDOWNLOADDATA then
                ...
                -- Логирование статуса ошибки загрузки
                sampAddChatMessage(scriptName .. "Ошибка загрузки. Статус: " .. tostring(status), 0xFF0000)
            end
        end)

        -- Проверяем, удалось ли скачать
        if success then
            return true
        else
            sampAddChatMessage(scriptName .. "Ошибка загрузки. Попытка " .. attempt, 0xFF0000)
            wait(delay) -- Задержка перед следующей попыткой
        end
    end
    return false
end
